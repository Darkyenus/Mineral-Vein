...
public class MineralVein extends JavaPlugin{
	...
	private class WorldApplier implements Runnable{
		...
		public void run(){
			...
			System.gc();
			out.printf("Applying MineralVein to "+w.getName()+". "+(chunksLength-chunks.size())+"/"+chunksLength+"%3.4f%%, %4.1fMB free   \r",((((chunksLength-chunks.size()))*100)/(double)chunksLength),((Runtime.getRuntime().freeMemory())/(double)(1024*1024)));
			if(chunks.size() == 0){
				out.print("\n");
